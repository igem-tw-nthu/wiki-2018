...
const Light =
    <div>
     <div className='part'>Initially, we tried to use white LED separately because it is cheap and low cost, besides, having the best degree of contrast. However, we found that it is not light enough, so we chose another kind of light.</div>
...
const  Temperature =
    <div>
        ...
        <div className='part'>
            ...
            <div className='inner-part'>Common methods for cooling semiconductors include water cooling and fan cooling. Among them, due to our large architecture, water cooling is effective but too cumbersome and heavy. Limited by the space configuration, we use fan cooling. (Picture 4) By keep bringing the heat away, the temperature gradient of the chip become larger, and finally it will stop at a balanced point. We also add a fan on the cold side since we want the cold air to be uniformed inside the box. (Picture 5) Temperature-controlled circuits are more cumbersome and too complicated circuits are not suitable for use in our system, so we try to use temperature control module first. (Picture 6)</div>
            <div className='illustration'>
                <img className='illust-image' src={Constant.image.environment.first}/>
                <div className='undertext'></div>
            </div>
            <div className='inner-part'>We found that as we put the cooling device in a cardboard box for model, the quality is not good that made the chip hard for cooling.</div>
            <div className='inner-part'>Needed materials: two 5V-2wire fans, two small heat sinks, some thermal grease, one thermoelectric cooling chip provided by a national competition in Taiwan, temperature control module.</div>
        </div>
        ...
        <div className='part'>
            ...
            <div className='inner-part'>To reduce the unknown impact on the experiment, we use two cold device and one hot device to control the temperature. Hot device is similar to cold one, the only different is its two fans are all 5V-2wire, which is the device we used in the first period.</div>
        　　<div className='inner-part'>This time we use two cold devices to find for the lowest temperature. Figure 1 shows the result we test cold or hot devices, and Figure 2 shows the changing after we turn off the devices.</div>
            <div className='illustration'>
